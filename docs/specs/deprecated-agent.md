...
| 42  | Re-exec `$0 "$AGENT_NAME" "$REPO_DIR" "$@"` after every iteration so canonical-script edits take effect on next launch | Edit `apps/agent-cli/bin/deprecated-agent` mid-session, exit; next iteration runs the new version. **Shim edits are NOT picked up by the restart loop** — the shim already `exec`-ed into the canonical script before the loop started, so `$0` at re-exec time is the canonical script, not the shim. Shim edits require a full session restart. (This is fine in practice — the shim is trivial and rarely edited.) Implementation note for PR-C: the re-exec MUST preserve `AGENT_NAME` and `REPO_DIR` as the first two positional args, since the shim only supplies them on the initial invocation. |
