...
| 5. Extract           | Copied to `plugins/claude-code/task-utils/{hooks,skills}/` in nsheaps/agents; adjusted `plugin.json` to declare the hook + skill ([PR #142](https://github.com/nsheaps/agents/pull/142), merged 2026-05-17). Marketplace-registration follow-up: [PR #145](https://github.com/nsheaps/agents/pull/145).                                                                               |
| 6. Replace           | **DONE for alex.** Commit [`2361d60`](https://github.com/nsheaps/.ai-agent-alex/commit/2361d60) dropped the `hooks` block from `.claude/settings.json` and deleted `bin/hooks/{task-invariant,require-task-in-progress}.sh` + `.claude/skills/manage-tasks/`. Tracked as alex task #68 (broken down into atomic #233 settings-edit and #234 file-delete).                                                                                                                                                                                                                                                                |
| 7. Revalidate        | **DONE for alex.** Post-delete `TaskUpdate(234, completed)` was blocked by a PreToolUse hook running `bash ${CLAUDE_PLUGIN_ROOT}/hooks/task-invariant.sh` — the local file is gone, so the plugin path is the only possible source. `Skill(manage-tasks)` now resolves to the plugin copy (the local copy at `.claude/skills/manage-tasks/` no longer exists).                                                                                                                                                                                                                                                          |
| 8. Iterate-on-plugin | Subsequent fixes (e.g., validation-steps parser tightening per [PR #143](https://github.com/nsheaps/agents/pull/143)) landed as plugin PRs in the regular review cycle. Phase 6 took longer than the ideal — alex stayed on the local copy for several plugin-PR cycles because local-works-fine kept the deferral cheap. The cost of NOT switching is silent drift between alex's local copy and the canonical plugin — caught here when this spec made a too-confident claim about Phase 6/7 being done, then resolved by actually completing Phase 6 (commit `2361d60` above) before merging this spec.            |

Total local iteration time: ~weeks. Total plugin-extraction PR cycle: ~1 day for Phase 5; Phase 6 (consumer-side cutover) was deferred for several plugin-PR cycles before completing in commit `2361d60`. The deferral itself was informative: once the plugin is shipping, the residual urgency to switch off local is low, because local works fine. The cost of NOT switching is silent drift between alex's local copy and the canonical plugin — caught when this spec originally made a too-confident claim about Phase 6/7 being done.
