...
| 6. Replace           | **PENDING for alex as of this writing.** task-utils is published in the nsheaps/agents marketplace but alex's `.claude/settings.json` still references `bin/hooks/task-invariant.sh` directly and `.claude/skills/manage-tasks/SKILL.md` still exists locally. Switching consumer-side is tracked separately (task #68) and is itself a real-world test of Phase 6/7. |
| 7. Revalidate        | Cannot validate yet because Phase 6 hasn't run. When it does, expected check: TaskCreate / TaskUpdate hooks still fire from the plugin's installed location, and `Skill(manage-tasks)` resolves to the plugin copy.                                                                                                                                  |
| 8. Iterate-on-plugin | Subsequent fixes (e.g., validation-steps parser tightening per [PR #143](https://github.com/nsheaps/agents/pull/143)) landed as plugin PRs in the regular review cycle. _Note that during this period, alex was still on the local copy, so the plugin PRs landed without alex having to restart — that ratio of "iterate-while-local" cost is part of why Phase 6 is still pending._ |

Total local iteration time: ~weeks. Total plugin-extraction PR cycle: ~1 day for Phase 5; Phase 6 (consumer-side cutover) deferred. The deferral itself is informative: once the plugin is shipping, the residual urgency to switch off local is low, because local works fine. The cost of NOT switching is silent drift between alex's local copy and the canonical plugin — caught here when this spec made a too-confident claim about Phase 6/7 being done.
