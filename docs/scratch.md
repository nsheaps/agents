...
- **Post-compaction check-in**: When an agent is working in a team with a team-lead, after compaction it should check in with the orchestrator to re-sync on current state, active tasks, and any instructions it may have lost. This needs to be a behavior baked into the agent launcher or agent definitions, not a global user rule.
- **Agent check-in protocol**: Before performing any tasks, an agent should check in with the orchestrator/team-lead. When a task is assigned, the agent should: (1) acknowledge receipt ("I got it"), and (2) confirm it actually started working ("I started working on it") — the second message should only come after work has actually begun. This protects against the case where the agent is doing work in the foreground and can't receive messages.
- **Foreground work is dangerous**: If an agent is working in the foreground (not via a Task/background agent), it cannot receive messages from any team members. All substantive work MUST go through the Task tool (background agents). Consider a PreToolUse hook that blocks non-Task tool usage for agents in team mode, forcing all implementation work into background agents. This ensures agents remain responsive to teammate messages at all times.
...
## Teammate Execution Modes & Permissions
...
- Consider teammates and their ability or allow permissions for (or gate based on settings) running tasks:
  ...
  - maybe need a max teammate launch depth?
  - they MUST use a 'background' mechanism to perform _ALL_ tasks (maybe hook to enforce this)
...
- **MCP message routing to sub-agents**: Local sub-agents (Task tool / background agents) must NOT receive MCP-injected messages — only the parent agent should get them. If a message is injected into a sub-agent's conversation, it gets trapped in that sub-agent's context and is effectively lost to the parent. The MCP server responsible for message injection needs to be aware of the agent hierarchy and only target the top-level session.
  - The MCP server should also track whether a session is active or idle. If idle, it should try alternate delivery — e.g., typing into the tmux frame — though this is a fragile fallback.
  - Better approach: idle detection at the stop hook triggers termination or restart of the agent, with the incoming message as the trigger for the new session. This avoids injecting messages into a dormant agent that's doing nothing.
  - This aligns well with print-mode (non-interactive), but interactivity still feels important — agents need to be responsive to messages, not just batch processors.
