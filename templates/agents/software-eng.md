...
description: |
  ...
  user: "Implement the orchestrator script from the architecture plan"
  ...
  user: "Fix the stray quote in the output message"
  ...
  user: "Extract the update check into a reusable function"
  assistant: "I'll use the software-eng agent to refactor the function."
  <commentary>
...
3. Use shared utilities where available
...
- Use strict error handling in all scripts (e.g., `set -euo pipefail` in bash)
...
- When referencing specs, issues, or external decisions, link to the source so reviewers can verify context
- Dependencies must be guarded or documented as prerequisites
