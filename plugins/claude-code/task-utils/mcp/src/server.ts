...
export function createServer(): McpServer {
    ...
    "task_create",
    {
      ...
      inputSchema: {
        ...
        metadata: z
          .record(z.any())
          .optional()
          .describe("Free-form metadata object."),
      },
    },
    async (args) => {
      try {
    ...
    "task_update",
    {
      ...
      inputSchema: {
        ...
        metadata: z
          .record(z.any())
          .optional()
          .describe("Metadata to merge into the task."),
      },
    },
    async (args) => {
      try {
  ...
  server.registerTool(
    "task_get",
    {
      title: "Get task",
      description:
        "Get a single task by id. Read-only. Mirrors the built-in TaskGet tool.",
      inputSchema: {
    ...
    async (args) => {
      try {
        ...
        if (!task) {
          return errorResult(
            new TaskError(`No task #${args.taskId} found in the task store.`),
          );
        }
