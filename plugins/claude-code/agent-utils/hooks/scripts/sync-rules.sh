...
#
# SessionStart hooks must never fail — wrap body in main() with `trap 'exit 0' ERR`
# so unexpected errors (e.g. mkdir -p permission denied) don't break the session start.
set -Eeuo pipefail  # -E so ERR trap propagates into main()
