...
- **security-1** (P4): Security grep catches common patterns (api_key, token, password, secret, Bearer, sk-, ghp_) but doesn't cover AWS keys (`AKIA...`), 1Password `op://` URIs with embedded secrets, or Azure keys. The instruction to "redact actual credential values" serves as a catch-all beyond the grep. Acceptable as a safety net.
