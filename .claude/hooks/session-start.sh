...
# mise.toml here declares no explicit `gh` pin but the repo relies on
# github-app for git identity, and nothing was running `mise
# install`/`mise activate` or persisting a PATH export for cloud sessions.
