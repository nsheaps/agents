...
beforeAll(async () => {
  // Build-if-missing: the integration test drives the COMPILED binary. When run
  // via `mise run test-task-mcp` the binary is already built (build-task-mcp
  // runs first); when run via `bun test` / `nx run-many --target=test` with no
  // ordering guarantee, compile it here so the test is self-contained.
  if (!existsSync(SERVER_BIN)) {
    execFileSync(
      "bun",
      ["build", "--compile", "--outfile", SERVER_BIN, join(HERE, "..", "src", "server.ts")],
...
describe("MCP server over stdio", () => {
  ...
  test("task_get reads a task back", async () => {
    ...
    const out = payload(res as never);
    expect((out.task as { subject: string }).subject).toBe("integration task one");
  });
  ...
  test("task_list returns all tasks and filters by status", async () => {
    const all = payload((await client.callTool({ name: "task_list", arguments: {} })) as never);
