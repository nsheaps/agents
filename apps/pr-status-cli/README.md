...
| `--all-open` / `--no-all-open`     | on        | when on (default), open PRs are always shown regardless of `--since`; `--since` only trims the closed/merged bucket  |

In digest mode the output is sorted per-repo:
1. **Open PRs** — by mergeability: `CLEAN` → `UNSTABLE` → `BEHIND` → other (descending PR# within each tier)
2. **Merged** 🟣 — descending PR#
3. **Closed-no-merge** ❌ — descending PR#
